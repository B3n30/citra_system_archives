...
VERSION = 0x03000000
#BF64: header sizes stay the same
...
class Bffnt:
    ...
    def read(self, filename):
        ...
        if self.invalid:
            return
        #BF64: deleting that stuff doesn't work \/
    ...
    def save(self, filename):
        ...
        data = struct.pack(FINF_HEADER_STRUCT % self.order, FINF_HEADER_MAGIC, FINF_HEADER_SIZE, font_info['fontType'],
                           font_info['lineFeed'], font_info['alterCharIdx'], default_width['left'], default_width['glyphWidth'],
                           default_width['charWidth'], font_info['encoding'], 0, 0, 0, font_info['height'], font_info['width'],
               font_info['ascent'], 0)
        file_.write(data)
        ...
        data = struct.pack(TGLP_HEADER_STRUCT % self.order, TGLP_HEADER_MAGIC, 0, tglp['glyph']['width'],
                           tglp['glyph']['height'], tglp['glyph']['baseline'],  tglp['maxCharWidth'], tglp_data_size,
                           tglp['sheetCount'], sheet['format'], sheet['cols'], sheet['rows'], sheet['width'],
...
#BF64: all of these lovely error messages and I don't get any of them
    ...
    def _parse_finf(self, data):
        magic, section_size, font_type, line_feed, alter_char_idx, def_left, def_glyph_width, \
            def_char_width, encoding, tglp_offset, cwdh_offset, cmap_offset, height, width, ascent, reserved \
    ...
    def _parse_tglp_header(self, data):
        magic, section_size, cell_width, cell_height, baseline_position, max_char_width, sheet_size, num_sheets, \
